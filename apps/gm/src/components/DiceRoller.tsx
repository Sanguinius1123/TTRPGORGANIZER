...
import { rollDiceGm, rollDicePlayer, getRecentRolls } from '@/lib/actions/diceRolls'
...
export function DiceRoller({ mode, campaignId, pcId, pcName }: DiceRollerProps) {
  ...
  useEffect(() => {
    ...
    if (campaignId) {
      getRecentRolls(campaignId).then(rolls => setRecentRolls(rolls))
    }
  }, [campaignId])
  ...
  const handleRoll = () => {
    ...
    startTransition(async () => {
      const result =
        mode === 'gm'
          ? await rollDiceGm(formData)
          : await rollDicePlayer(formData, pcId ?? '', pcName ?? 'Player')
