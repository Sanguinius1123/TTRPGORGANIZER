  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-2 mb-6 text-sm">
        <Link href="/play/factions" className="text-slate-500 hover:text-slate-300">Factions</Link>
            ...
            <ul className="divide-y divide-slate-700/50">
              {members.map(({ npc, role }) => (
                <li key={npc.id} className="px-6 py-3 flex items-center justify-between">
                  <Link href={`/play/npcs/${npc.id}`} className="text-sm font-medium text-indigo-400 hover:underline">
                    {npc.name}
                  </Link>
