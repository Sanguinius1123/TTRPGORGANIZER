...
export default async function NPCDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const results = await Promise.all([
    supabase.from('npcs').select('id, name, species, profession, culture, background, disposition, notes, image_url, visible, current_location_id, campaign_id, created_at').eq('id', id).eq('visible', true).single(),
