      ...
      {locations.length === 0 ? (
        ...
        <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {locations.map(loc => (
                <ClickableRow key={loc.id} href={`/play/locations/${loc.id}`} className="border-b border-slate-700/50 last:border-0 hover:bg-slate-800">
                  <td className="px-4 py-3">
                    <SubLink href={`/play/locations/${loc.id}`} className="font-medium text-slate-100 hover:text-indigo-400">
                      {loc.name}
                    </SubLink>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {loc.parent
                      ? <SubLink href={`/play/locations/${loc.parent.id}`} className="text-slate-500 hover:text-indigo-400">{loc.parent.name}</SubLink>
