  ...
  return (
    <div className="p-8 max-w-2xl space-y-10">
      ...
      <section className="space-y-3">
        ...
        <p className="text-sm text-slate-400">
          Deleting a campaign permanently removes it and all its entities — locations, NPCs, factions, sessions, lore, and everything else. This cannot be undone.
        </p>
        {campaigns.length === 0 ? (
              ...
              <tbody>
                {campaigns.map(c => (
                  <tr key={c.id} className="border-b border-slate-700 last:border-0">
                    <td className="px-4 py-2.5 text-slate-100 font-medium">{c.name}</td>
                    <td className="px-4 py-2.5 text-slate-500 text-xs max-w-xs truncate">{c.description ?? '—'}</td>
