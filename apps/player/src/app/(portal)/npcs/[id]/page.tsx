  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-2 mb-6 text-sm">
        <Link href="/npcs" className="text-slate-500 hover:text-slate-300">NPCs</Link>
                ...
                <div>
                  ...
                  <dd className="text-sm text-slate-100">
                    {speciesIdByName[npc.species]
                      ? <Link href={`/species/${speciesIdByName[npc.species]}`} className="text-indigo-400 hover:underline">{npc.species}</Link>
                      : npc.species}
                  </dd>
                </div>
              )}
                ...
                <div>
                  ...
                  <dd className="text-sm text-slate-100">
                    {cultureIdByName[npc.culture]
                      ? <Link href={`/cultures/${cultureIdByName[npc.culture]}`} className="text-indigo-400 hover:underline">{npc.culture}</Link>
                      : npc.culture}
                  </dd>
                </div>
              )}
              ...
              {npcFactionRows.map(row => {
                ...
                return (
                  <li key={row.faction_id} className="px-6 py-3 flex items-center justify-between">
                    <Link href={`/factions/${faction.id}`} className="text-sm font-medium text-indigo-400 hover:underline">
                      {faction.name}
                    </Link>
